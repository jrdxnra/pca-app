...
export default function ProgramsPage() {
    ...
    // State for Quick Workout Builder (Grid Clicks)
    ...
    // Handler for when schedule event cards are clicked - prefer linked workout;
    // fallback to same-day workout lookup when metadata link is missing.
    const handleScheduleEventClick = async (event: GoogleCalendarEvent) => {
        ...
        // Infer client from event summary when metadata is missing.
        ...
        // Last-resort fallback: probe all clients for workouts on this day and
        // use a unique match when event metadata is absent.
        ...
        // If event has a linked workout, navigate to builder to edit it
        if (linkedWorkoutId) {
        ...
        // No workout linked - navigate to builder with eventId (builder will open Quick Workout dialog)
        const params = new URLSearchParams();
        params.set('date', dateParam);
        params.set('eventId', event.id);
    ...
    return (
        <div className="w-full px-1 pt-1 pb-4 space-y-2">
                ...
                <CardContent className="py-1 px-2">
                    <div className="flex flex-wrap items-center justify-between gap-1 gap-y-4">
                        ...
                        <div className="flex items-center gap-1 lg:gap-2">
                            ...
                            <div className="min-w-[110px] text-center font-medium text-sm px-2">
                                {getNavigationLabel()}
                            </div>
                        </div>

                        {/* Right aligned - Actions & Tools */}
                            ...
                            {selectedClient && (
                                <>
                                    ...
                                    >
                                        ...
                                        Manage<span className="hidden 2xl:inline"> Periods</span>
                        ...
                        <React.Suspense fallback={<CalendarSkeleton includeWeekends={includeWeekends} />}>
                            <ModernCalendarView
                                ...
                                onWorkoutCellClick={(date: Date, timeSlot: Date, period?: ClientProgramPeriod) => {
                                    // When clicking workout cell, open builder dialog
            ...
            {/* Quick Workout Builder Dialog (Grid Clicks) */}
            {(selectedDateForWorkout || quickWorkoutDialogOpen) && (
                <QuickWorkoutBuilderDialog
                    ...
                    initialDate={selectedDateForWorkout ? format(selectedDateForWorkout, 'yyyy-MM-dd') : undefined}
                    initialTime={selectedTimeForWorkout ? format(selectedTimeForWorkout, 'HH:mm') : undefined}
                    onClose={() => {
